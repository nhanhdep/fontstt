...
//var dataStorge = ;
var firstText = "Nguyễn Thanh Cần";
...
var getHistory = function () {
    if (arrLocal != null && arrLocal.length > 0) {
        $("div#history").append("<h4>Gần đây nhất</h4>");
        for (var i = 0; i < arrLocal.length; i++) {
            ...
            $(".input-group-append span").text("Sao chép");
...
var firstData = function () {
    ...
    for (var i = 0; i < 89; i++) {
        ...
        var span_action = $("<span data-type='button' class='input-group-text copybutton' data-clipboard-action='copy' data-clipboard-target='#copy_" + i + "'></span>").text("Sao chép");
...
$(function () {
    ...
    $(".loadmore").click(function () {
        ...
        var that = $(this);
        var intvl = setInterval(function () { that.html('Xem thêm'); clearInterval(intvl); }, 1000);
        for (var i = 1; i <= 10; i++) {
            ...
            //action
            var _div_action = $("<div class='input-group-append'></div>")
            var span_action = $("<span data-type='button' class='input-group-text copybutton' data-clipboard-action='copy' data-clipboard-target='#copy_" + ct + "'></span>").text("Sao chép");
            _div_action.append(span_action)
            _divGr.append(_div_action);
            //
            ct++;
    ...
    clipboard.on('success', function (e) {
        ...
        $("#snackbar").text("Đã sao chép: " + e.text);
